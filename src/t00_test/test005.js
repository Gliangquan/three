...
// 4,创建点光源
var pointLight = new THREE.PointLight(0xff0000, 100);
...
smallBall.add(pointLight);  // 将点光源作为子物体添加到小球上
smallBall.position.set(5, 5, 5);  // 设置小球和光源的位置
scene.add(smallBall);
...
function render() {
    ...
    smallBall.position.x = Math.sin(time) * 3 + 3;
    smallBall.position.z = Math.cos(time) * 3 + 3;
    smallBall.position.y = Math.sin(time) + 3
